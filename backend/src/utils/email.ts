...
export async function sendEmail(to: string, subject: string, html: string, options: { requireDelivery?: boolean } = {}): Promise<void> {
  if (!env.resend.apiKey) {
    if (options.requireDelivery && env.nodeEnv === "production") throw new Error("Email delivery is not configured");
  ...
  if (!response.ok) {
    if (options.requireDelivery) throw new Error("Email provider rejected the request");
